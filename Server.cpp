...
void printEscaped(const std::string& buf) {
...
int Server::checkPassword(int clientSocket)
{
	...
	size_t findEscape = received.find_first_of("\r\n");
	while (findEscape != std::string::npos)
	{
		std::cout << findEscape << std::endl;
		received.erase(findEscape);
		printEscaped(received);
		findEscape = received.find_first_of("\r\n");
	}
	// received.erase(received.size());
	// std
	// buffer[n] = '\0';
	...
	{
		std::cout << received << " wrong password inputted... " << std::endl;
